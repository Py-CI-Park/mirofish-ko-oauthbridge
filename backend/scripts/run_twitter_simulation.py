...
이 스크립트는 설정 파일의 매개변수를 읽어 시뮬레이션을 실행하고 전체 과정을 자동화합니다.
...
class UnicodeFormatter(logging.Formatter):
    """Unicode 이스케이프 시퀀스를 읽을 수 있는 문자로 변환하는 사용자 지정 포매터입니다."""
...
def setup_oasis_logging(log_dir: str):
    """고정 파일 이름을 사용하는 OASIS 로그를 설정합니다."""
...
class TwitterSimulationRunner:
    ...
    def _get_profile_path(self) -> str:
        """Profile 파일 경로를 가져옵니다(OASIS Twitter는 CSV 형식 사용)."""
    ...
    def _create_model(self):
        ...
        프로젝트 루트 .env 파일의 설정을 일관되게 사용합니다(최우선):
        - LLM_API_KEY: API 키
    ...
    async def run(self, max_rounds: int = None):
        ...
        if not os.path.exists(profile_path):
            print(f"오류: Profile 파일이 없습니다: {profile_path}")
            return
        ...
        if initial_posts:
            print(f"초기 이벤트 실행 중(초기 게시물 {len(initial_posts)}개)...")
            initial_actions = {}
            ...
            if initial_actions:
                ...
                print(f"  초기 게시물 {len(initial_actions)}개 게시됨")
        ...
        if self.wait_for_commands:
            print("\n" + "=" * 60)
            print("명령 대기 모드 진입 - 환경은 계속 실행됩니다")
...
async def main():
    ...
    # 로그 설정 초기화(고정 파일 이름 사용, 기존 로그 정리)
...
def setup_signal_handlers():
    ...
    프로그램이 리소스(데이터베이스, 환경 등)를 정상적으로 정리할 기회를 줍니다.
    """
