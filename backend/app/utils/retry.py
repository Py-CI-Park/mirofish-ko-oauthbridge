"""
API 호출 재시도 유틸리티
LLM 등 외부 API 호출에 대한 재시도 로직을 제공한다.
"""
...
def retry_with_backoff(
    ...
    Args:
        ...
        jitter: 난수 지연을 추가할지 여부
        exceptions: 재시도 대상 예외 유형
        on_retry: 재시도 시 호출할 콜백 (exception, retry_count)
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    logger.warning(
                        ...
                        f"{current_delay:.1f}초 후 다시 시도합니다..."
                    )
...
class RetryableAPIClient:
    ...
    def call_batch_with_retry(
        ...
        """
        여러 항목을 처리하되, 각 실패 항목은 개별적으로 재시도한다.
        
        Args:
            items: 처리할 항목 목록
            process_func: 단일 item을 인자로 받는 처리 함수
            exceptions: 재시도 대상 예외 유형
            continue_on_failure: 한 항목 실패 후에도 계속 진행할지 여부
