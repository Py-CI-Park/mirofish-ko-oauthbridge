"""Zep Graph 페이지 조회 유틸리티.

Zep의 node/edge 목록 API는 UUID cursor 기반 페이지네이션을 사용한다.
이 모듈은 단일 페이지 재시도를 포함한 자동 페이징 로직을 감싸고,
호출자에게는 전체 목록을 투명하게 반환한다.
"""
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """단일 페이지를 요청하고 실패 시 지수 백오프로 재시도한다. 네트워크/IO 성격의 일시적 오류만 재시도한다."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """그래프 노드를 페이지 단위로 조회한다. 최대 max_items개(기본 2000개)까지 반환하며, 각 페이지 요청에는 재시도가 포함된다."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """그래프의 전체 엣지를 페이지 단위로 조회해 반환한다. 각 페이지 요청에는 재시도가 포함된다."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
