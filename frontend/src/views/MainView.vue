...
const checkBridgeBeforeOntology = async () => {
  try {
    ...
    if (bridge.busy) {
      ...
      addLog(`로컬 OAuth 브리지가 사용 중입니다. 온톨로지 생성 요청을 대기열에 넣습니다${queued > 0 ? ` (${queued}건 앞에 있음)` : ''}.`)
      ontologyProgress.value = {
        message: queued > 0
          ? `로컬 OAuth 브리지가 사용 중입니다. 현재 요청 뒤에 ${queued}건 대기 중입니다...`
          : '로컬 OAuth 브리지가 사용 중입니다. 현재 요청이 끝날 때까지 기다리는 중입니다...'
      }
    }
  } catch (err) {
    throw new Error(
      err?.message ||
      '로컬 OAuth 브리지에 연결할 수 없습니다. dev:all이 실행 중인지 확인해 주세요.'
    )
  }
}
